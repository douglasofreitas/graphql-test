...
export const postResolvers = {
    ...
    Query: {
        ...
        post: (partent, {id}, context: ResolverContext, info: GraphQLResolveInfo) => {
            id = parseInt(id);
            return context.db.Post
                .findById(id, {
                    attributes: context.requestedFields.getFields(info, {keep: ['id'], exclude:['comments']})
            ...
            return db.sequelize.transaction((t: Transaction) => {
                return db.Post
                    ...
                    .then((post: PostInstance) => {
                        throwError(!post, `Post with id ${id} not found`);
                        throwError(post.get('author') != authUser.id, `Unautoried! You can only edit posts created by yourself.`);
                        input.author = authUser.id;
        ...
        deletePost: compose(...authResolvers)((parent, {id}, {db, authUser}: {db: DbConnection, authUser: AuthUser}, info: GraphQLResolveInfo) => {
            ...
            return db.sequelize.transaction((t: Transaction) => {
                return db.Post
                    ...
                    .then((post: PostInstance) => {
                        throwError(!post, `Post with id ${id} not found`);
                        throwError(post.get('author') != authUser.id, `Unautoried! You can only edit posts by yourself.`);
